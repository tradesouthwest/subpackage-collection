...
/**
 * Subpackage_Collection_Settings Class
 *
...
final class Subpackage_Collection_Settings {
	...
	public function render_field ( $args ) {
		...
		// Construct the key.
		$key = Subpackage_Collection()->token . '-' . $args['section'] . '[' . $args['id'] . ']';
	...
	public function get_settings_sections () {
		...
		$settings_sections['standard-fields'] = __( 'Standard Fields', 'subpackage-collection' );
		$settings_sections['special-fields']  = __( 'Special Fields', 'subpackage-collection' );
		...
		switch ( $section ) {
			case 'standard-fields':
				$settings_fields['text']     = array(
					'name'        => __( 'Example Text Input', 'subpackage-collection' ),
					'type'        => 'text',
					'default'     => '',
					'section'     => 'standard-fields',
					'description' => __( 'Place the field description text here.', 'subpackage-collection' ),
				);
				$settings_fields['textarea'] = array(
					'name'        => __( 'Example Textarea', 'subpackage-collection' ),
					'type'        => 'textarea',
					'default'     => '',
					'section'     => 'standard-fields',
					'description' => __( 'Place the field description text here.', 'subpackage-collection' ),
				...
				$settings_fields['checkbox'] = array(
					'name'        => __( 'Example Checkbox', 'subpackage-collection' ),
					'type'        => 'checkbox',
					'default'     => '',
					'section'     => 'standard-fields',
					'description' => __( 'In this example checkbox is used for debugging (removes send buffer of zlib output error).', 'subpackage-collection' ),
				);
				$settings_fields['radio']    = array(
					'name'        => __( 'Example Radio Buttons', 'subpackage-collection' ),
					'type'        => 'radio',
					'default'     => '',
					'section'     => 'standard-fields',
					'options'     => array(
						'one'   => __( 'One', 'subpackage-collection' ),
						'two'   => __( 'Two', 'subpackage-collection' ),
						'three' => __( 'Three', 'subpackage-collection' ),
					),
					'description' => __( 'Place the field description text here.', 'subpackage-collection' ),
				);
				$settings_fields['select']   = array(
					'name'        => __( 'Example Select', 'subpackage-collection' ),
					'type'        => 'select',
					'default'     => '',
					'section'     => 'standard-fields',
					'options'     => array(
			...
			case 'special-fields':
				$settings_fields['select_taxonomy'] = array(
					'name'        => __( 'Example Taxonomy Selector', 'subpackage-collection' ),
	...
	protected function render_field_text ( $key, $args ) {
		$html = '<input id="' . esc_attr( $key ) . '" name="' . esc_attr( $key ) . '" 
		size="40" type="text" value="' . esc_attr( $this->get_value( $args['id'], $args['default'], $args['section'] ) ) . '" />' . "\n";
		return $html;
	}
	...
	protected function render_field_radio ( $key, $args ) {
		...
		if ( isset( $args['options'] ) && ( 0 < count( (array) $args['options'] ) ) ) {
			...
			foreach ( $args['options'] as $k => $v ) {
				$html .= '<input type="radio" name="' . esc_attr( $key ) . '" 
				value="' . esc_attr( $k ) . '"' . checked( esc_attr( $this->get_value( $args['id'], $args['default'], $args['section'] ) ), $k, false ) . ' /> ' . esc_html( $v ) . '<br />' . "\n";
			}
		}
		return $html;
	}
	...
	protected function render_field_checkbox ( $key, $args ) {
		...
		$html .= '<input id="' . esc_attr( $key ) . '" name="' . esc_attr( $key ) . '" 
		    type="checkbox" value="true"' . checked( esc_attr( $this->get_value( $args['id'], 
			$args['default'], $args['section'] ) ), 'true', false ) . ' />' . "\n";
		if ( $has_description ) {
	...
	public function get_value ( $key, $default, $section ) {
		$values = get_option( 'subpackage-collection-' . $section, array() );
	...
	public function get_settings ( $section = '' ) {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				...
				$values = get_option( 'subpackage-collection-' . $v, array() );
